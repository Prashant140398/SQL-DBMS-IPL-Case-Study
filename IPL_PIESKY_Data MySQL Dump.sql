...
SELECT * FROM IPL_MATCH SCHEDULE;
...
FROM IPL_BIDDER_DETAILS LEFT JOIN IPL_BIDDING_DETAILS ON IPL_BIDDER_DETAILS.BIDDER_ID=IPL_BIDDING_DETAILS.BIDDER_ID 
...
select a.BIDDER_ID,d.bidder_name,case when b.TOTAL_POINTS=0 then 0
else count(*)
end counting ,b.NO_OF_MATCHES,(case when b.TOTAL_POINTS=0 then 0
else count(*)
end/b.NO_OF_BIDS)*100 c from ipl_bidding_details a  inner join 
ipl_bidder_points b on a.bidder_id=b.BIDDER_ID inner join
ipl_bidder_details d on d.BIDDER_ID=a.BIDDER_ID
where  a.bid_status="won" or b.TOTAL_POINTS=0 group by a.bidder_id order by c desc;
-- Q2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
SELECT MS.STADIUM_ID,S.STADIUM_NAME,S.CITY,COUNT(*)
FROM IPL_MATCH_SCHEDULE MS INNER JOIN IPL_STADIUM S ON MS.STADIUM_ID=S.STADIUM_ID
...
ELSE 0 END))/COUNT(*)*100 AS 'PERCENTAGE OF WINS' 
FROM IPL_STADIUM IST JOIN IPL_MATCH_SCHEDULE MS ON IST.STADIUM_ID=MS.STADIUM_ID JOIN IPL_MATCH IM ON MS.MATCH_ID=IM.MATCH_ID 
 GROUP BY IST.STADIUM_NAME 
 ORDER BY 1;

-- Q4.	Show the total bids along with bid team and team name.
SELECT BID_TEAM,TEAM_NAME,COUNT(NO_OF_BIDS)
FROM IPL_BIDDER_POINTS BP LEFT JOIN IPL_BIDDING_DETAILS BD ON BP.BIDDER_ID=BD.BIDDER_ID LEFT JOIN IPL_TEAM TEAM ON BD.BID_TEAM=TEAM.TEAM_ID
...
FROM IPL_TEAM TEAM INNER JOIN IPL_TEAM_STANDINGS TS ON TEAM.TEAM_ID=TS.TEAM_ID LEFT JOIN IPL_TOURNAMENT IT ON TS.TOURNMT_ID=IT.TOURNMT_ID 
GROUP BY TEAM_NAME; 
-- Q7.	Display the bowlers for Mumbai Indians team.
SELECT P.PLAYER_ID,PLAYER_NAME
FROM IPL_TEAM TEAM LEFT JOIN IPL_TEAM_PLAYERS TEAMP ON TEAM.TEAM_ID=TEAMP.TEAM_ID LEFT JOIN IPL_PLAYER P ON TEAMP.PLAYER_ID=P.PLAYER_ID
